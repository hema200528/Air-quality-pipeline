...
def connect_db(db_path: Union[str, Path]) -> duckdb.DuckDBPyConnection:
    ...
    Raises:
        ConnectionError: If connection to DuckDB database fails.
    """
    try:
...
def create_raw_schema(con: duckdb.DuckDBPyConnection) -> None:
    ...
    try:
        ...
        logger.error(f"Failed to create raw database schema: {e}")
        raise
...
def load_raw_data(con: duckdb.DuckDBPyConnection, parquet_file: Union[str, Path]) -> None:
    ...
    Raises:
        ...
        duckdb.Error: If data loading fails.
    """
    ...
    try:
        ...
        logger.error(f"Failed to load raw data into DuckDB: {e}")
        raise
...
def run_benchmark_queries(con: duckdb.DuckDBPyConnection) -> None:
    ...
    try:
        ...
        logger.error(f"Failed benchmark execution: {e}")
        raise
...
def setup_served_database(
    ...
    Raises:
        duckdb.Error: If served database creation/load fails.
    """
    try:
        con = duckdb.connect(str(served_db_path))
    except Exception as e:
        logger.error(f"Failed to connect/create serving database '{served_db_path}': {e}")
        raise
    ...
    try:
        ...
        logger.error(f"Failed to populate serving database: {e}")
        raise
    finally:
        con.close()
